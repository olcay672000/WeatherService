...
public interface IWeatherInfoRepository extends CrudRepository<WeatherInfo, Long> {
    boolean existsByPlaceInfoAndDatetimeAndStationName(@Param("pi") PlaceInfo pi,
    ...
    Iterable<WeatherInfo> findByPlaceId(@Param("placeId") String placeId);

    Iterable<WeatherInfo> findByPlaceInfo(@Param("placeInfo") PlaceInfo placeInfo);
    ...
    @Query(value = "select w.place_info_id, w.weather_information_id, w.lng, w.observation, w.iCAO, w.clouds, w.dew_point, w.clouds_code, " +
            ...
            "from weather_information w inner join place_info pi on pi.place_info_id = w.place_info_id " +
            "where p.place_name = :placeName and w.place_info_id = pi.place_info_id and date_part('hour', w.date_time) = :hour and date_part('minute', w.date_time) = :minute",
             nativeQuery = true)
